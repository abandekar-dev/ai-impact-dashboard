...
# Add system Python packages to path
sys.path.insert(0, '/nix/store/rv55xcjw63fcfb8p9qd85s8hfxjry3w5-python3.11-streamlit-1.34.0/lib/python3.11/site-packages')
...
try:
    ...
    def show_overview():
        ...
        with col3:
            st.metric("Active Projects", len([
                init for func_data in st.session_state.ai_initiatives.values() 
                for init in func_data.values()
            ]))
        
        with col4:
            avg_roi = sum(
                init.get('expected_roi', 0) 
                for func_data in st.session_state.ai_initiatives.values() 
                for init in func_data.values()
            ) / max(1, sum(len(func_data) for func_data in st.session_state.ai_initiatives.values()))
        ...
        # Quick stats
        if st.session_state.ai_initiatives:
            st.subheader("Quick Statistics")
            
            # Function distribution
            function_counts = {func: len(initiatives) for func, initiatives in st.session_state.ai_initiatives.items() if initiatives}
            if function_counts:
    ...
    def show_executive_summary():
        ...
        for function, initiatives in st.session_state.ai_initiatives.items():
            if initiatives:
                ...
                total_func_investment = sum(init['investment'] for init in initiatives.values())
                avg_roi = sum(init['expected_roi'] for init in initiatives.values()) / len(initiatives)
                
                col1, col2 = st.columns(2)
                with col1:
                    st.write(f"Total Investment: ${total_func_investment:,.0f}")
                with col2:
                    st.write(f"Average ROI: {avg_roi:.1f}%")
                
                for name, data in initiatives.items():
                    st.write(f"  • {name}: ${data['investment']:,.0f} ({data['timeline']} months, {data['expected_roi']}% ROI)")
    ...
    print("Streamlit is not available. Please install required dependencies.")
    sys.exit(1)